...
namespace yunggh
{
    public class ShortestWalk : GH_Component
    {
        public ShortestWalk()
            : base("Shortest Walk", "WALK",
            "Calculates the shortest walk in a network of curves",
            "yung gh", "Machine Learning")
        { }

        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            pManager.AddCurveParameter("Curves", "C", "The curves group", GH_ParamAccess.list);
            pManager.AddNumberParameter("Lengths", "L", "One length for each curve. If the number of lengths is less than the one of curves, length values are repeated in pattern.\nIf there are no lengths, then the physical length of the curves is computed.", GH_ParamAccess.list);
            pManager[1].Optional = true;
            pManager.AddLineParameter("Wanted path", "P", "The lines from the start to the end of the path", GH_ParamAccess.list);
        }
        ...
        private static Predicate<Curve> _removeNullAndInvalidDelegate = RemoveNullAndInvalid;
        private static Predicate<Line> _removeInvalidDelegate = RemoveInvalid;
        private static Predicate<double> _isNegative = IsNegative;
